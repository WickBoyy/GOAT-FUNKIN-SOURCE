	...
	**/
	...
	public static function setLanguage(?name:String):Void {
		...
		for(l in Paths.assetsTree.libraries) {
			var mod = AssetsLibraryList.getCleanLibrary(l);
	...
	public static function loadLanguage(lang:String):Map<String, IFormatInfo>
	{
		...
		for(l in Paths.assetsTree.libraries) {
			var mod = AssetsLibraryList.getCleanLibrary(l);
			if(mod is TranslatedAssetLibrary) continue;

			if(mod is ICustomAssetLibrary) {
				var lib = cast(mod, ICustomAssetLibrary);
				for(file in lib.getFiles("assets/" + mainPath).sortAlphabetically().map((v)->'$mainPath/$v')) {
