...
final class AudioAnalyzer
{
	...
	{
		if (wordSize == 2)
			return inline ArrayBufferIO.getInt16(buffer, position);
		else if (wordSize == 3)
		{
			...
			return inline ArrayBufferIO.getInt32(buffer, position);
		else
			return inline ArrayBufferIO.getUint8(buffer, position) - 128;
	}
			...
			maxDb = -10.0, minFreq = 20.0, maxFreq = 22000.0):Array<Float>
	{
		...
		for (i in 0...barCount)
		{
			...
			if (ratio > 0 && ratio < 1 && v < levels[i])
				levels[i] -= (levels[i] - v) * ratio;
			else
				levels[i] = v;
		}
	...
	public static function getFrequenciesFromSamples(samples:Array<Float>, fftN = 2048, useWindowing = false, ?frequencies:Array<Float>):Array<Float>
	{
		...
		if (log == 0)
			throw "AudioAnalyzer.getFrequenciesFromSamples: Cannot insert a fftN of 1";

		var i = log - 1;
		fftN = 1 << log;

		#if (target.threaded) __mutex.acquire(); #end
		var reals:Array<Float> = __freqReals[__freqCalculating],
			imags:Array<Float> = __freqImags[__freqCalculating];
		if (reals == null)
		{
		...
		while (i > 0)
		{
			i--;
			if (useWindowing)
				reals[reverseIndices[i]] = samples[i] * windows[i];
			else
				reals[reverseIndices[i]] = samples[i];
			imags[i] = 0;
		}
	...
	function __check()
		if (sound != null && sound.buffer != buffer)
		{
	...
	public function getLevels(?startPos:Float, ?volume:Float, barCount:Int, ?levels:Array<Float>, ?ratio:Float, ?minDb:Float, ?maxDb:Float, ?minFreq:Float,
		...
		return inline getLevelsFromFrequencies(__frequencies = getFrequencies(startPos, volume, __frequencies), buffer.sampleRate, barCount, levels, ratio,
	...
	public function getFrequencies(?startPos:Float, ?volume:Float, ?frequencies:Array<Float>):Array<Float>
		return inline getFrequenciesFromSamples(__freqSamples = getSamples(startPos != null ? startPos : sound.time, fftN, true, -1, volume, __freqSamples),
			fftN, useWindowingFFT, frequencies);
	...
	public function analyze(startPos:Float, endPos:Float, ?outOrOutMin:Array<Float>, ?outMax:Array<Float>):Float
	{
		...
		if (hasOut)
		{
			...
			if (hasTwoOut)
				for (i in 0...buffer.channels)
				{
					if (outOrOutMin[i] < (f = __min[i] / byteSize))
						outOrOutMin[i] = f;
					if (outMax[i] < (f = __max[i] / byteSize))
						outMax[i] = f;
				}
			else
			{
				outOrOutMin.resize(2);
				if (outOrOutMin[0] < (f = __minByte / byteSize))
					outOrOutMin[0] = f;
				if (outOrOutMin[1] < (f = __maxByte / byteSize))
					outOrOutMin[1] = f;
			}
		}
	...
	public function getSamples(startPos:Float, length:Int, mono = true, channel = -1, volume = 1.0, ?output:Array<Float>, ?outputMerge = false):Array<Float>
	{
		...
		__check();
		__read(startPos, startPos + (length / __toBits * buffer.channels),
			mono ? __getSamplesCallbackMono : (channel == -1 ? __getSamplesCallback : __getSamplesCallbackChannel));

		__sampleOutput = null;
		return output;
	}

	function __getSamplesCallbackMono(b:Int, c:Int):Void
		if (__sampleIndex < __sampleOutputLength)
		{
			if (c == 0)
			{
				if (__sampleOutputMerge)
	...
	function __getSamplesCallbackChannel(b:Int, c:Int):Void
		if (__sampleIndex < __sampleOutputLength)
		{
	...
	function __getSamplesCallback(b:Int, c:Int):Void
		if (__sampleIndex < __sampleOutputLength)
		{
			if (__sampleOutputMerge)
				__sampleOutput[__sampleIndex] += b * __sampleToValue;
	...
	{
		if (buffer.data != null)
			__readData(startPos, endPos, callback);
		#if lime_cffi
		else if (__canReadStream() && (startPos += __readStream(startPos, endPos, callback)) >= endPos)
		{
		}
		#if lime_vorbis
		else if (__prepareDecoder())
			__readDecoder(startPos, endPos, callback);
		#end
		#end
	}
	...
	inline function __readStream(startPos:Float, endPos:Float, callback:AudioAnalyzerCallback):Float @:privateAccess {
		...
		while (++i < backend.bufferLengths.length)
			if (startPos >= (time = backend.bufferTimes[i] * 1000))
			{
	...
	inline function __prepareDecoder():Bool @:privateAccess {
		if (buffer.__srcVorbisFile == null)
			return __vorbis != null;
		if (__vorbis != null)
			return true;
		if ((__vorbis = buffer.__srcVorbisFile.clone()) != null)
		{ // IM HOPING IT HAVE A GC CLOSURE.
