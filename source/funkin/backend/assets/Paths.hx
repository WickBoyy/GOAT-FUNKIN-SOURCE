...
class Paths
{
	...
	{
		FlxG.signals.preStateSwitch.add(function() tempFramesCache.clear());
	}
	...
	**/
	...
	public static function getFolderDirectories(key:String, addPath:Bool = false):Array<String>
	{
		if (!key.endsWith("/"))
			key += "/";
		var content = assetsTree.getFolders('assets/$key');
		if (addPath)
		{
			for (k => e in content)
				content[k] = '$key$e';
	...
	static public function getFolderContent(key:String, addPath:Bool = false, noExtension:Bool = false):Array<String>
	{
