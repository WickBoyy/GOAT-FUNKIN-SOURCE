...
class Chart {
	...
	public static function loadChartMeta(songName:String, ?variant:String, ?difficulty:String, includeMetaVariations = true):ChartMetaData {
		...
		if (data.difficulties.length <= 0) {
			...
			data.difficulties = [for(f in Paths.getFolderContent(path, false)) if (Path.extension(f.toUpperCase()) == "JSON") Path.withoutExtension(f)];
		...
		if (includeMetaVariations && data.variants.length > 0) for (variant in data.variants) {
			if (!data.metas.exists(variant) && Assets.exists(Paths.file('songs/$songName/meta-$variant.json'))) {
				var meta = loadChartMeta(songName, variant);
	...
	public static function parse(songName:String, ?difficulty:String, ?variant:String):ChartData {
		...
		var base:ChartData = {
			...
			codenameChart: true,
			fromMods: Paths.assetsTree.existsSpecific(chartPath, "TEXT")
		};
		...
		var loadedMeta = loadChartMeta(songName, variant, difficulty, false);
