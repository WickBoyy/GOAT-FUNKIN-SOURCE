...
class CharterNoteTypesList extends UISubstateWindow {
	...
	public override function create() {
		...
		var noteTypesFileList = getNoteTypesList();
		if (noteTypesFileList.length == 0) noteTypesFileList = getNoteTypesList();
	...
	public function getNoteTypesList() {
		var list:Array<String> = [];
		for (path in Paths.getFolderContent(pathString, true)) if(Path.extension(path) == "hx") {
