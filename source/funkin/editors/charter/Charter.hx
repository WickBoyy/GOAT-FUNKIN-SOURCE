...
class Charter extends UIState
{
	...
	public function updateCameraChanges()
	{
		...
		for (grp in [leftEventsGroup, rightEventsGroup])
		{
			...
			for (e in grp.members)
			{
				for (event in e.events)
				{
					...
					else if (event.name == "Center Camera")
					{
						cameraMovementChanges.push({
							strumLineID: -1,
							step: e.step,
		...
		cameraMovementChanges.sort(function(e1, e2) return FlxSort.byValues(FlxSort.ASCENDING, e1.step, e2.step));
	...
	public function updateAutoSaving(elapsed:Float)
	{
		...
		if (autoSaveTimer <= 0)
		{
			...
			if (!autoSaveNotif.cancelled)
			{
				...
				UIState.playEditorSound('editors/save');
				undos.save();
			}
			...
			case NOTE_DRAG:
				...
				if (selectionDragging)
				{
					...
					}, function(e:CharterEvent)
					{
						...
						e.setPosition(e.eventsBackdrop.x
							+ (e.global ? 0 : e.eventsBackdrop.width - e.bWidth)
				...
				else
				{
					...
					if (!(verticalChange == 0 && horizontalChange == 0))
					{
						...
						undos.addToUndo(CChangeBundle([CSelectionDrag(undoDrags), updateEventsGroups(selection)]));
					}
