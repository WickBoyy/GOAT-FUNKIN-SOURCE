...
class Character extends FunkinSprite implements IBeatReceiver implements IOffsetCompatible implements IPrePostDraw {
	...
	public static function getList(includeFolders:Bool = false, folder:String = 'data/characters/'):Array<String> {
		var list:Array<String> = [];
		if(includeFolders) {
			for (path in Paths.getFolderDirectories(folder, true)) {
				if(!path.endsWith("/")) path += "/";
				list.push(path);
			}
		}
		for (path in Paths.getFolderContent(folder, true))
