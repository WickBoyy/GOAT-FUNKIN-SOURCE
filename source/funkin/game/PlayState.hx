...
class PlayState extends MusicBeatState
{
	...
	**/
	...
	/**
	 * The additional zoom for the game camera (used for bops).
	 */
	public var gameBopZoom:Float = 0;

	/**
	 * The additional zoom for the HUD camera (used for bops).
	 */
	public var hudBopZoom:Float = 0;

	/**
	 * The time it takes for the camera to return to its default zoom.
	 */
	public var camBopTime(get, never):Float;

	private function get_camBopTime():Float
	{
		var time = ((Conductor.stepCrochet * 4) / 1000) * camZoomingInterval;
		if (FlxMath.roundDecimal(time, 3) <= 0.333)
			time *= 2;
		return time;
	}

	/**
	 * The ease function for the camera bop.
	 */
	public var camBopEase:EaseFunction = FlxEase.quintOut;
	...
	private inline function get_maxCamZoom()
		return Math.isNaN(maxCamZoom) ? defaultCamZoom + (maxCamZoomMult * camGameZoomMult) : maxCamZoom;
	...
	@:dox(hide) override public function create()
	{
		...
		// PRECACHING
		#if REGION
		for (content in Paths.getFolderContent('images/game/score/', true))
	...
	#if VIDEO_CUTSCENES
	@:noCompletion
	private function makeVid(vidName:String, noAudio:Bool):hxvlc.flixel.FlxVideoSprite
	{
		var video = new hxvlc.flixel.FlxVideoSprite(0, 0);
		video.load(openfl.utils.Assets.getPath(Paths.video(vidName)), [(noAudio ? ':no-audio' : '')]);
	...
	@:dox(hide) private function generateSong(?songData:ChartData):Void
	{
		...
		if (events == null)
			...
			events = [
				for (e in songData.events)
				{
					switch (e.name)
					{
						case "Camera Movement", "Center Camera":
						...
						case "Play Video":
							...
							video.load(Assets.getPath(Paths.video(e.params[0])), [':no-audio']);
							video.play();
							video.visible = false;
							add(video);
							#end
		...
		events.sort(function(p1, p2) return FlxSort.byValues(FlxSort.DESCENDING, p1.time, p2.time));
	...
	override public function update(elapsed:Float)
	{
		...
		if (video != null)
		{
			...
			if (video.bitmap != null)
				video.bitmap.rate = FlxG.timeScale;
		}
		#end
		...
		if (Options.camZoomOnBeat && camZooming)
		{
			...
			if (camZoomingLastBeat != beat)
			{
				...
				{
					gameBopZoom += camGameZoomMult * camZoomingStrength;
					hudBopZoom += camHUDZoomMult * camZoomingStrength;
					FlxTween.cancelTweensOf(this, ["gameBopZoom", "hudBopZoom"]);
					FlxTween.tween(this, {gameBopZoom: 0, hudBopZoom: 0}, camBopTime, {ease: camBopEase});
				}
			}
		}
		...
		if (camZooming)
		{
			FlxG.camera.zoom = defaultCamZoom + gameBopZoom;
			camHUD.zoom = defaultHudZoom + hudBopZoom;
		}
	...
	{
		var strumLine = strumLines.members[curCameraTarget];
		if (strumLine != null)
		{
			var data:CamPosData = getStrumlineCamPos(curCameraTarget);
			if (data.amount > 0)
			{
				var event = gameAndCharsEvent("onCameraMove", EventManager.get(CamMoveEvent).recycle(data.pos, strumLine, data.amount));
				if (!event.cancelled)
	...
	public function executeEvent(event:ChartEvent) @:privateAccess {
		...
		switch (event.name)
		{
			...
			case "Play Video":
				...
				video = vidList.get(vidName) ?? makeVid(vidName, !event.params[1]);
			...
			case "Center Camera":
				...
				camFollow.setPosition((dadPos.x + bfPos.x) * 0.5, (dadPos.y + bfPos.y) * 0.5);

				if (event.params[0] == false)
				{
					FlxG.camera.snapToTarget();
				}
				else if (event.params[2] != null && event.params[2] != "CLASSIC")
				{
					var oldFollow = FlxG.camera.followEnabled;
					FlxG.camera.followEnabled = false;
					...
					eventsTween.set("cameraMovement", FlxTween.tween(FlxG.camera.scroll, {
						...
						(Conductor.stepCrochet / 1000) * (event.params[1] == null ? 4 : event.params[1]), {
							ease: CoolUtil.flxeaseFromString(event.params[2], event.params[3]),
			...
			case "Add Camera Zoom":
				if (event.params[1] == "camHUD")
					hudBopZoom += event.params[0];
				else
					gameBopZoom += event.params[0];
			...
			case "Camera Bop":
				...
				{
					gameBopZoom += event.params[0] * camZoomingStrength;
					hudBopZoom += event.params[0] * camZoomingStrength;
					FlxTween.cancelTweensOf(this, ["gameBopZoom", "hudBopZoom"]);
					FlxTween.tween(this, {gameBopZoom: 0, hudBopZoom: 0}, camBopTime, {ease: camBopEase});
				}
			case "Camera Zoom":
				...
				if (event.params[0] == false)
				{
					...
					eventsTween.set(name,
						FlxTween.tween(this, (cam == camHUD ? {defaultHudZoom: finalZoom} : {defaultCamZoom: finalZoom}),
	...
	public function goodNoteHit(strumLine:StrumLine, note:Note):Void
	{
		...
		if (strumLine != null && !strumLine.cpu)
			event = EventManager.get(NoteHitEvent)
				.recycle(false, !note.isSustainNote, !note.isSustainNote, null, defaultDisplayRating, defaultDisplayCombo, note, strumLine.characters, true,
					note.noteType,
					...
					"game/score/", "", note.strumID, rating.score, note.isSustainNote ? null : rating.accuracy, 0.023,
					rating.name, Options.splashesEnabled && !note.isSustainNote && rating.splash, 0.5, true, 0.7, true, true, iconP1);
		else
			event = EventManager.get(NoteHitEvent)
				.recycle(false, false, false, null, defaultDisplayRating, defaultDisplayCombo, note, strumLine.characters, false, note.noteType,
					note.animSuffix.getDefault(note.strumID < strumLine.members.length ? strumLine.members[note.strumID].animSuffix : strumLine.animSuffix),
					"game/score/", "", note.strumID, 0, null, 0, rating.name, false, 0.5, true, 0.7, true, true, iconP2);
