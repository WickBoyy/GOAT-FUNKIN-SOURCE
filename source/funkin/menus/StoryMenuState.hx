...
class StoryWeeklist
{
	...
	public function getWeeksFromSource(useTxt:Bool = true, loadCharactersData:Bool = true)
	{
		var path:String = Paths.txt('weeks/weeks');
		var weeksFound:Array<String> = useTxt && Paths.assetsTree.existsSpecific(path, "TEXT") ? CoolUtil.coolTextFile(path) : [
			for (c in Paths.getFolderContent('data/weeks/weeks/', false)) if (Path.extension(c).toLowerCase() == "xml") Path.withoutExtension(c)
		];
	...
	public static function get(useTxt:Bool = true, loadCharactersData:Bool = true)
	{
		...
		weekList.getWeeksFromSource(useTxt, loadCharactersData);
		return weekList;
	}
}
