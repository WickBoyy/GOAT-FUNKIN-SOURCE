...
class FreeplaySonglist {
	...
	public function getSongsFromSource(useTxt:Bool = true) {
		var songsFound:Array<String> = null;
		if (useTxt) {
			var oldPath = Paths.txt('freeplaySonglist');
			var newPath = Paths.txt('config/freeplaySonglist');
			if (Paths.assetsTree.existsSpecific(newPath, "TEXT")) songsFound = CoolUtil.coolTextFile(newPath);
			else if (Paths.assetsTree.existsSpecific(oldPath, "TEXT")) {
		...
		if (songsFound == null) songsFound = Paths.getFolderDirectories("songs", false);
		if (songsFound.length > 0) {
			for (s in songsFound) songs.push(Chart.loadChartMeta(s));
			return false;
		}
		return true;
	}
	...
	public static function get(useTxt:Bool = true) {
		...
		songList.getSongsFromSource(useTxt);
		return songList;
	}
}
