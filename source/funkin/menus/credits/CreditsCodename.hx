...
class CreditsCodename extends funkin.options.TreeMenuScreen
{
	...
	override function update(elapsed:Float)
	{
		...
		else if (_canReset && PlayerSettings.solo.controls.RESET)
			tryUpdating();
	}
	...
	public function tryUpdating(forceDisplaying:Bool = false)
	{
		...
		Main.execAsync(() ->
		{
			if (checkUpdate() || forceDisplaying)
				_downloadingSteps = 2;
			else
				_downloadingSteps = 1;
		});
	...
	public function updateMarkup()
	{
		...
		parent.descLabel.applyMarkup(text, contribFormats = [
			...
			new FlxTextFormatMarkerPair(new FlxTextFormat(FlxColor.interpolate(Flags.MIN_CONTRIBUTIONS_COLOR, Flags.MAIN_DEVS_COLOR,
	...
	override function close()
	{
		for (frmt in contribFormats)
			parent.descLabel.removeFormat(frmt.format);
		super.close();
	}
	...
	public function checkUpdate():Bool
	{
		...
		if (Options.lastUpdated != null && curTime < Options.lastUpdated + 120000)
			return false; // Fuck you Github rate limits  - Nex
	...
	public function displayList()
	{
		...
		if (curSelected > Options.contributors.length - 1)
			curSelected = Options.contributors.length - 1;
		...
		for (c in Options.contributors)
		{
			var text = TU.translate("credits.totalContributions", [
				c.contributions,
				totalContributions,
				FlxMath.roundDecimal(c.contributions / totalContributions * 100, 2)
			]);
