...
class CreditsMain extends TreeMenu {
	...
	override function create() {
		...
		for (l in Paths.assetsTree.libraries) {
			var lib = AssetsLibraryList.getCleanLibrary(l);
			if (lib is ICustomAssetLibrary) {
				var i = cast(lib, ICustomAssetLibrary).libName;
	...
	// XML STUFF
	public function parseCreditsFromXML(xml:Access):Array<FlxSprite> {
				...
				switch(node.name) {
					case "credit":
						var opt:PortraitOption = new PortraitOption(name, desc, function() if(node.has.url) CoolUtil.openURL(node.att.url),
							node.has.icon && Paths.assetsTree.existsSpecific(Paths.image('credits/${node.att.icon}'), "IMAGE") ?
					...
					case "menu":
						credsMenus.push(new TextOption(name, desc, ' >', () -> addMenu(new TreeMenuScreen(name, desc, parseCreditsFromXML(node)))));
				}
			}
		}
